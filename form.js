...
function Redirect() {
 ...
 function ValidMail() {
      ...
      if (!valid) {
        output = 'Адрес eл. пошти введено неправильно!';
        document.getElementById('span').innerHTML = output + "<br>";
        return false;
      } else {
        return true;
      }
    ...
    function ValidPhone() {
      var output2;
      var re = /^\d[\d\(\)\ -]{4,14}\d$/;
      var myPhone = document.getElementById('phon_number').value;
      var valid = re.test(myPhone);
      if (!valid) {
        output2 = 'Номер телефона введено неправильно!';
        document.getElementById('span2').innerHTML = output2 + "<br>";
        return false;
      } else {
        return true;
      }
...
document.write('<tr><td colspan="7" align="center" style="cursor:pointer;font:13px Arial;background:white" onclick="_Calendar.today()">Сьогодні: '+_Calendar.addnull(_Calendar.sccd,_Calendar.sccm+1,_Calendar.sccy)+'</td></tr>');
